...
void WebviewWindowPlugin::HandleMethodCall(
  ...
  if (method_call.method_name() == "create") {
    ...
    // Get windowIcon parameter if provided
    std::string windowIcon = "";
    if (arguments->find(flutter::EncodableValue("windowIcon")) != arguments->end()) {
      auto iconValue = arguments->at(flutter::EncodableValue("windowIcon"));
      if (!iconValue.IsNull()) {
        windowIcon = std::get<std::string>(iconValue);
      }
    }
    ...
    window->CreateAndShow(
        ...
        int(windowPosX), int(windowPosY), useWindowPositionAndSize, openMaximized, utf8_to_wide(windowIcon),
